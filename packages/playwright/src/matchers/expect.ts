...
class ExpectMetaInfoProxyHandler implements ProxyHandler<any> {
  ...
  get(target: Object, matcherName: string | symbol, receiver: any): any {
    ...
    return async (...args: any[]) => {
      ...
      const step = await testInfo._addStep(stepInfo);

      const reportStepError = async (e: Error | unknown) => {
        ...
        if (jestError?.matcherResult.suggestedRebaseline) {
          ...
          await step.complete({ suggestedRebaseline: jestError?.matcherResult.suggestedRebaseline });
          return;
        }
        await step.complete({ error });
