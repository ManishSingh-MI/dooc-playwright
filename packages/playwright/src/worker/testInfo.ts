...
import type { FullProject, Page, TestInfo, TestStatus, TestStepInfo } from '../../types/test';
...
export interface TestStepInternal {
  complete(result: { error?: Error | unknown, suggestedRebaseline?: string }): Promise<void>;
...
export class TestInfoImpl implements TestInfo {
  ...
  public fixtureRunner?: FixtureRunner;
  ...
  async _addStep(data: Omit<TestStepInternal, 'complete' | 'stepId' | 'steps' | 'attachmentIndices' | 'info'>, parentStep?: TestStepInternal): Promise<TestStepInternal> {
      ...
      complete: async result => {
        ...
        try {
          if (![
            ...
            const object = await this.fixtureRunner?.resolveParametersForFunction(this.fn, this, 'test', { type: 'test' });
            const page: Page = (object as { page: Page }).page;
            if (page && this.title) {
              const body = await page.screenshot({ path: `./test-results/step-end/${this.title.toLowerCase().replaceAll(' ', '_')}.png` });
              this._onAttach({
                path: `./test-results/step-end/${this.title.toLowerCase().replaceAll(' ', '_')}.png`,
                testId: this.testId,
                name: `${stepId} :: stepend:: ${data.title}`,
          ...
          // console.log('>> ERROR Params page:', data.title, data.location);
        }
    ...
    // attach a screenshot before step
    try {
      if (![
        'browserContext.newPage',
        'browser.newContext',
        ...
        // console.log('>> Params page:', stepId, data.title, data.location);

        const object = await this.fixtureRunner?.resolveParametersForFunction(this.fn, this, 'test', { type: 'test' });

        const page: Page = (object as { page: Page }).page;
        if (page && this.title) {
          const body = await page.screenshot({ path: `./test-results/step-begin/${this.title.toLowerCase().replaceAll(' ', '_')}.png` });
          this._onAttach({
            path: `./test-results/step-begin/${this.title.toLowerCase().replaceAll(' ', '_')}.png`,
            testId: this.testId,
            name: `${stepId} :: stepbegin:: ${data.title}`,
            contentType: 'image/png',
            body: body?.toString('base64'),
            stepId,
          });
  ...
  async _runAsStage(stage: TestStage, cb: () => Promise<any>) {
    ...
    stage.step = stage.stepInfo ? (await this._addStep({ ...stage.stepInfo, title: stage.title, isStage: true })) : undefined;
    ...
    try {
      ...
      await stage.step?.complete({ error });
      throw error;
    } finally {
  ...
  async attach(name: string, options: { path?: string, body?: string | Buffer, contentType?: string } = {}) {
    const step = await this._addStep({
      title: `attach "${name}"`,
      category: 'attach',
    });
    ...
    try {
      ...
      // console.log('catch step.complete err', err);
    }
  }
